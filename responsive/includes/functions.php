...
if (!function_exists('responsive_setup')):
    ...
    function responsive_setup() {
         ...
         * This feature enables custom-menus support for a theme. Mod partner menu.
        ...
        register_nav_menus(array(
			...
			'partner-menu'      => __('Partner Menu', 'responsive'),
...
function responsive_breadcrumb_lists() {
	...
	/*eb--additional custom sidebars*/



	register_sidebar(array(

            'name' => __('Right Sidebar-custom', 'responsive'),

            'description' => __('Area 2 - sidebar-right-custom.php', 'responsive'),
	...
	register_sidebar(array(
            ...
            'name' => __('Right Sidebar-custom-school', 'responsive'),

            'description' => __('Area 2 - sidebar-right-custom-school.php', 'responsive'),

            'id' => 'right-sidebar-custom-school',
	...
	/*eb--end additional custom sidebars*/
...
add_filter( 'pre_update_option_show_on_front', 'responsive_front_page_override', 10, 2 );
    ...
    /* EB MOD Exclude a Category from Search Results */
    ...
    $query->set( 'category__not_in' , array( 5 ) ); // Cat ID
...
function eitc_script($filingStatus, $kids) { 
    $eitc_parms = array(
        1=>array(
           "credit_rate"=>.34,
            "beginning_point"=>9720,
            "end_point"=>array(
                "single"=>17830,
                "married"=>23260
            ),
    ...
    <!-- Formula for <?php echo $filingStatus; ?> with <?php echo $kids; ?> Kids -->

        if (EARNINGS < <?php echo $eitc_parms[$kids]["beginning_point"]; ?>){
            EITC = Math.round(100*(EARNINGS * <?php echo $eitc_parms[$kids]["credit_rate"]; ?>)/100)
        }

        if (EARNINGS > <?php echo $eitc_parms[$kids]["beginning_point"]-1; ?>)
        if (EARNINGS < <?php echo $eitc_parms[$kids]["end_point"][$filingStatus]; ?>){
            EITC = <?php echo $eitc_parms[$kids]["max_credit"]; ?>
        }

        if (EARNINGS > <?php echo $eitc_parms[$kids]["end_point"][$filingStatus]-1; ?>)
        if (EARNINGS < <?php echo $eitc_parms[$kids]["income_cut_off"][$filingStatus]; ?>){
            EITC = Math.round(100*(<?php echo $eitc_parms[$kids]["max_credit"]; ?> - (<?php echo $eitc_parms[$kids]["phase_out_rate"]; ?> * (EARNINGS - <?php echo $eitc_parms[$kids]["end_point"][$filingStatus]; ?>)))/100)
        };

        if (EARNINGS >= <?php echo $eitc_parms[$kids]["income_cut_off"][$filingStatus]; ?>){EITC = "Not Eligible"}
